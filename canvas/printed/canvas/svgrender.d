...
public:
    ...
    {
        output(format(
            `<rect x="%s" y="%s" width="%s" height="%s" stroke="%s" stroke-width="%s" stroke-dasharray="%-(%f %)" stroke-dashoffset="%f" fill="none"/>`,
            convertFloatToText(x),
            convertFloatToText(y),
            convertFloatToText(width),
            convertFloatToText(height),
            _currentStroke,
